...
public class SaleControllerTest {
    ...
    class PostSalesTest {
        ...
        void shouldReturnValidationErrorForInvalidSaleEnabled() throws Exception {
            // Arrange
            String json = """
                    {
                       "saleEnabled": null,
    ...
    class PutSalesTest {
        ...
        void shouldUpdateSaleSuccessfully() throws Exception {
            // Arrange
            Long saleId = 1L;
            SaleDTORequest saleDTORequest = new SaleDTORequest(true,1L,1L,saleDetailDTORequestsList);
            SaleDTOResponse updateResponse = new SaleDTOResponse(1L,new BigDecimal("200.00"),new BigDecimal("36.20"),true,clientDTOResponse,userDTOResponse,saleDetailDTOResponsesList);

            when(saleService.update(eq(saleId), any(SaleDTORequest.class))).thenReturn(updateResponse);

            // Act & Assert
            mockMvc.perform(put(APISALES+"/"+saleId)
                            .contentType(MediaType.APPLICATION_JSON)
                            .content(objectMapper.writeValueAsString(saleDTORequest)))
                    .andExpect(status().isOk())
                    .andExpect(content().contentType(MediaType.APPLICATION_JSON))
                    .andExpect(jsonPath("$.saleId").value(1L))
                    .andExpect(jsonPath("$.saleTotal").value(200.00))
                    .andExpect(jsonPath("$.saleTax").value(36.20))
                    .andExpect(jsonPath("$.saleEnabled").value(true))
            ...
            // Verify
            verify(saleService,times(1)).update(eq(saleId), any(SaleDTORequest.class));
        }


        // update fail
        @Test
        @DisplayName("should return 404 when update non-existent sale")
        void shouldReturnNotFoundWhenUpdatingNonExistSale() throws Exception {
            // Arrange
            Long saleIdNonExist = 99L;
            SaleDTORequest saleDTORequest = new SaleDTORequest(true,1L,1L,saleDetailDTORequestsList);

            when(saleService.update(eq(saleIdNonExist), any(SaleDTORequest.class))).thenThrow(new ResourceNotFoundException(MESSAGE_NOT_FOUND));

            // Act & Assert
            mockMvc.perform(put(APISALES+"/"+saleIdNonExist)
                            .contentType(MediaType.APPLICATION_JSON)
                            .content(objectMapper.writeValueAsString(saleDTORequest)))
                    .andExpect(status().isNotFound())
                    .andExpect(jsonPath("$.status").value(404))
                    ...
                    .andExpect(jsonPath("$.path").value(APISALES+"/"+saleIdNonExist))
                    .andExpect(jsonPath("$.timestamp").exists());

            // Verify
            verify(saleService,times(1)).update(eq(saleIdNonExist), any(SaleDTORequest.class));
            verifyNoMoreInteractions(saleService);
        }
        // @Valid saleEnabled
        @Test
        @DisplayName("should return 400 when saleEnabled is invalid on update")
        void shouldReturnValidationErrorForInvalidSaleEnabledOnUpdate() throws Exception {
            // Arrange
            Long saleId = 1L;
            String json = """
                    {
            ...
            mockMvc.perform(put(APISALES+"/"+saleId)
                    ...
                    .andExpect(jsonPath("$.message").value(containsString("saleEnabled")))
                    .andExpect(jsonPath("$.message").value(containsString(ERROR_REQUIRED)))
                    .andExpect(jsonPath("$.path").value(APISALES+"/"+saleId))
                    .andExpect(jsonPath("$.timestamp").exists());
        ...
        // @Valid clientId
        @ParameterizedTest
        @DisplayName("should return 400 when clientId is invalid on Update")
        @MethodSource("provideInvalidClientIdOnUpdate")
        void shouldReturnValidationErrorForInvalidClientIdOnUpdate(String invalid,String expectedMessageFragment ) throws Exception {
            // Arrange
            Long saleId = 1L;
            String json = """
                    {
        ...
        private static Stream<Arguments> provideInvalidClientIdOnUpdate() {
            return Stream.of(
                    Arguments.of("null",ERROR_REQUIRED),
                    Arguments.of("-1",ERROR_POSITIVE)
            );
        ...
        // @Valid userId
        @ParameterizedTest
        @DisplayName("should return 400 when userId is invalid on Update")
        @MethodSource("provideInvalidUserIdOnUpdate")
        void shouldReturnValidationErrorForInvalidUserIdOnUpdate(String invalid,String expectedMessageFragment ) throws Exception {
            // Arrange
            Long saleId = 1L;
            String json = """
                    {
        ...
        private static Stream<Arguments> provideInvalidUserIdOnUpdate() {
            return Stream.of(
                    Arguments.of("null",ERROR_REQUIRED),
                    Arguments.of("-1",ERROR_POSITIVE)
            );
        ...
        // @Valid details
        @ParameterizedTest
        @DisplayName("should return 400 when Details is invalid on Update")
        @MethodSource("provideInvalidDetailsOnUpdate")
        void shouldReturnValidationErrorForInvalidDetailsOnUpdate(String invalid,String expectedMessageFragment ) throws Exception {
            // Arrange
            Long saleId = 1L;
            String json = """
                    {
        ...
        private static Stream<Arguments> provideInvalidDetailsOnUpdate() {
            return Stream.of(
                    Arguments.of("null",ERROR_REQUIRED),
                    Arguments.of("[]","List min 1 ")
            );
        ...
        // @Valid details / quantity
        @ParameterizedTest
        @DisplayName("should return 400 when DetailsQuantity is invalid On Update")
        @MethodSource("provideInvalidDetailsQuantityOnUpdate")
        void shouldReturnValidationErrorForInvalidDetailsQuantityOnUpdate(String invalid,String expectedMessageFragment ) throws Exception {
            // Arrange
            Long saleId = 1L;
            String json = """
                    {
        ...
        private static Stream<Arguments> provideInvalidDetailsQuantityOnUpdate() {
            return Stream.of(
                    Arguments.of("null",ERROR_REQUIRED),
                    Arguments.of("-1",ERROR_POSITIVE)
            );
        ...
        // @Valid details / productId
        @ParameterizedTest
        @DisplayName("should return 400 when DetailsProductId is invalid On Update")
        @MethodSource("provideInvalidDetailsProductIdOnUpdate")
        void shouldReturnValidationErrorForInvalidDetailsProductIdOnUpdate(String invalid,String expectedMessageFragment ) throws Exception {
            // Arrange
            String json = """
                    {
                       "saleEnabled": true,
        ...
        private static Stream<Arguments> provideInvalidDetailsProductIdOnUpdate() {
            return Stream.of(
                    Arguments.of("null",ERROR_REQUIRED),
                    Arguments.of("-1",ERROR_POSITIVE)
            );
    ...
    @Nested
    @DisplayName("DELETE /sales/{id}")
    class DeleteSalesTest {
        ...
        void shouldDeleteSaleSuccessfully() throws Exception {
            // Arrange
            Long saleId = 1L;
            doNothing().when(saleService).deleteById(saleId);
            // Act & Assert
            mockMvc.perform(delete(APISALES+"/"+saleId))
                    .andExpect(status().isNoContent());

            // Verify
            verify(saleService, times(1)).deleteById(saleId);
        }

        // fail delete
        @Test
        @DisplayName("should return 404 when deleting non existing sale")
        void shouldReturn404WhenDeletingNonExistingSale() throws Exception {
            // Arrange
            Long saleIdNonExist = 99L;
            doThrow(new ResourceNotFoundException(MESSAGE_NOT_FOUND)).when(saleService).deleteById(saleIdNonExist);
            // Act & Assert
            mockMvc.perform(delete(APISALES+"/"+saleIdNonExist))
                    .andExpect(status().isNotFound())
                    .andExpect(jsonPath("$.status").value(404))
                    .andExpect(jsonPath("$.error").value("Not Found"))
                    .andExpect(jsonPath("$.errorType").value("ResourceNotFound"))
                    ...
                    .andExpect(jsonPath("$.timestamp").exists());
            // Verify
            verify(saleService, times(1)).deleteById(saleIdNonExist);
        }
    }
    // LGTM!!!
}
